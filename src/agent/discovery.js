...
- A business_outcome may carry a "detail": a locator (and optional pattern) naming the line where the app states its own reason. Detect on the stable banner the app shows for a whole CLASS of rejection, then point detail at the specific message inside it — that way one rule covers every rejection and the caller still learns which one it was. Reading it is best-effort and can never turn the outcome back into a failure.
- A business_outcome whose resolution needs a PERSON rather than the caller — a supervisor override, an authority this operator does not hold — is declared the same way but with escalate: true. Replay then stops and reports ESCALATED with the screen and the step, instead of handing the caller an answer it cannot act on. Use it for permission refusals; do not use it for ordinary rejections the caller could fix by calling again with different arguments.
- Some legitimate endings do not belong to a single step. "No such member" shows up one step AFTER the search that caused it; a maintenance page, an expired session or a permission refusal can land on any step at all. Declare those at the top level in business_outcomes rather than on a step. They are checked only when a step is about to be reported as a failure, so they cannot mask a step that worked, and they are the right home for anything the flow can hit anywhere.
 ...
 * @param {string} args.appId   folder name under apps/
