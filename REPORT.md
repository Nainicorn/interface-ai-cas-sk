...
- Anthropic SDK — powers the discovery agent, and optionally one bounded fallback call during replay (section 7)

Express exposes one API that Playwright acts through, Zod makes sure whatever gets recorded from a Playwright run is a valid capability, and the Anthropic SDK only touches replay at all when a caller explicitly opts into assisted fallback — off by default, keeping replays deterministic
...
3. If the check fails, the system tries a short list of known fixes (`src/engine/recovery-table.js`), things like closing a popup or waiting out a slow load. These are fixed rules written ahead of time, not something the AI decides in the moment. `engine/replay.js` itself still never imports anything AI-related — that claim stayed true even after I added assisted fallback (section 7), which reaches replay only as an opaque, opt-in callback the file doesn't know is a model.
...
Secondarily, on UI drift: a `HARD_FAILURE` tells you the page changed enough to actually break something, but says nothing about a page that's slowly drifting while still technically working. `src/engine/drift.js` (section 7 has the full writeup) closes that gap — every replay after the first compares the page against a frozen baseline and logs a warning when it's diverged meaningfully, whether or not the replay itself still succeeds.
...
- Assisted fallback *was* cut originally — see the stretch-goal paragraph below for why I went back and built it, and how it stays bounded.
...
Beyond the assignment's own stretch-goal list, I also built one feature of my own: UI drift detection (`src/engine/drift.js`). A capability's first successful replay establishes a fingerprint of the page state after each step — the deduplicated, sorted set of accessibility-tree lines, so reordering the same elements never counts as drift. Every replay after that compares against that fixed baseline, and if a step's page has changed enough (a threshold tuned to tolerate one changed value among many, tested against a realistic-sized snapshot in `tests/drift.test.js`), it logs a `drift_warning` evidence event and surfaces it in the run report — without touching the outcome. A capability whose checkpoint still holds is still `SUCCESS`; drift is a heads-up that the ground moved, not a new way to fail. I verified this live: replayed a recording against its original page to establish a baseline, then replayed the identical recording against a deliberately reworked page (different nav, different copy, same functional button) and confirmed it still reported `SUCCESS` while correctly flagging drift on the affected steps. The gap this closes: a fixed, one-time recording can't tell you when it's started running against a page that quietly moved out from under it — this is the first hint you'd get, before a real break.

And the assisted-fallback stretch goal — the one I was most hesitant about, since it's the one piece that reintroduces non-determinism into replay, which is the property the whole system is built around. I kept every constraint from the original plan: `engine/replay.js` still imports nothing from the Anthropic SDK — `tests/invariants.test.js` still passes unmodified — because the model call lives entirely in a new file, `src/agent/assisted-fallback.js`, and reaches replay only as an opaque callback function replay never has to know is an LLM. It's off unless a caller explicitly opts in (`--assisted-fallback` on the CLI, `assisted_fallback: true` in the replay API body, or a per-capability checkbox in the console — unchecked by default). When on, it fires **at most once per replay, only when a step's locator can't be resolved at all**, and it can suggest nothing but an alternative locator for the same element — never a different action, url, or checkpoint — validated back against the schema before anything acts on it. Whatever it suggests is retried through the same action primitives as everything else, so `checkAllowed()` still runs; there is no privileged path. I tested the bounding properties directly — a stub confirms it never fires twice even when two steps in the same replay both fail — and then verified the real thing end to end: recorded a step against a button that had since been renamed, replayed once with the flag off (`HARD_FAILURE`, as expected) and once with it on, and the real model correctly identified the renamed button from the accessibility tree and recovered the run.
