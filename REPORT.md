...
With more time, next steps would be making the allowlist block by default instead of relying on every app being fully configured, and building the assisted-fallback stretch goal (a single bounded, policy-checked LLM recovery step) behind an explicit opt-in flag — the one stretch goal still deliberately not built, since it's the one piece that would reintroduce non-determinism into replay if done carelessly.
