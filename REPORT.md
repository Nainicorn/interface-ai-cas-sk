...
**Discovery has three stopping conditions**, per §3.1. Max steps escalates to a human. A
dead end — a goal the app genuinely cannot do, or one a human has said is impossible — ends
the run as `unreachable` with the agent's reasoning, via an `abandon` tool deliberately
distinct from `escalate`: escalation suspends and waits, abandonment is a verdict. Without
it, every hand-back topped the turn budget up and an impossible goal looped forever. There
is no wall-clock **timeout** — the third condition — and that is a cut: a paused run waits
for a human indefinitely, which suits a single-operator console and not production.
